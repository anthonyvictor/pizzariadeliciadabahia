...
import { LanchesViewStyle } from "./styles";
...
import { LancheItem } from "./item";
import { Lista } from "../lista";
import { Footer } from "src/views/loja/components/listas/footer";

export const LanchesView = () => {
  const { lanches, setEditando } = useLanches();
  const [search, setSearch] = useState("");
  // const categorias = Array.from(new Set(lanches.map((x) => x.categoria)));
  // const [categoriasFiltro, setCategoriasFiltro] = useState<string[]>([]);
  const [statsFiltro, setStatsFiltro] = useState<
    { tipo: "visivel" | "disponivel" | "somenteEmCombos"; valor: boolean }[]
  >([]);
  const filtrados = lanches?.length
    ? fuzzySearch(lanches, search, [
  ...
  return (
    <LanchesViewStyle>
      ...
      <ul className="categorias no-scroll">
        ...
        <Status label="Disponibilidade" tipo="disponivel" onClick={() => {}} />
      ...
      <Lista name="lanches">
        {filtrados.map((item) => {
          return <LancheItem key={item.id} item={item} />;
        })}
      </Lista>
      <Footer itens={lanches} filtrados={filtrados} />
      <FloatButton
        onClick={() => {
          setEditando("");
        }}
