...
import { PedidoViewStyle } from "./styles";
...
export const PedidoView = () => {
  const { editando, pedidos, setPedidos, setEditando } = usePedidos();
  const [carregando, setCarregando] = useState(false);
  const [formData, setFormData] = useState<IPedido>({
    ...{
      nome: "",
      imagemUrl: "",
      descricao: "",
  ...
  const handleSubmit = async () => {
    ...
    // const data = await salvar("/pedidos", "pedidos", [formData]);
    // if (data) {
    //   setPedidos((prev) => mergeArraysByKey(prev, data, "id"));
    //   setEditando(undefined);
    // }
    // setCarregando(false);
  };
