...
const Sabores: NextPage = () => {
  ...
  const { duasPequenas, promosCarregadas } = usePromo();
  ...
  const valorSaborFixo = comCoca ? 26.5 : comGoob ? 29 : 19; //27.5;
  const valorPromo = 37.99;
    ...
    <SaboresStyle>
      <p className="title">
        {comCoca || comGoob ? (
          ...
          <>
            <h5 className="title">2 pizzas {"PEQUENAS"} por:</h5>
      ...
      {groups.length && size ? (
        <>
          <TextContainer
            title="SABORES"
            subtitle={size && `SELECIONE APENAS 1 SABOR`}
          />
          <Searchbar
            value={search}
            setValue={setSearch}
      ...
      {showModalTuto && (
        <Modal
          ...
          description={`Selecione apenas um sabor e depois, clique em "Continuar!"`}
          type={"custom"}
          buttons={
            <>
              <ButtonPrimary
